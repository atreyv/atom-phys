...
dname = 'Z:/Data/2014/July/MOT/23_07_14/n_loadingtime_fluo/'
#prefix = 'tof'
...
#loading times
loading_times = np.array([100,300,500,700,900,1000,1500,2000,2500,3000,3500,5000,6000])
...
def atom_number(image): 
        ...
        N = np.float
        E = 2.54*10**(-19) #photon energy in joules
        eta = 1.19*10**(-10) #conversion of P from pixelheight to mW
        omega = 9.77*10**(-4) #solid angle
        ...
        power_pixel=np.sum(image[centre[0]-2.*sig_y:centre[0]+2.*sig_y,centre[1]-2.*sig_x:centre[1]+2.*sig_x])
        #intensity of each beam is 3.8 mW/cm^2 for 205 mW MOT beam, detuning=-2.8G
        scatt_rate=(10**6)*scattering_rate(22.8,2.8)
...
def number_time(Data,loading_times):
    ...
    #plot all the points
    plt.scatter(loading_times,numbers,label='N')
    plt.xlabel('t (ms)')
    plt.ylabel('N')
