...
dname = '/home/pedro/Dropbox/PhD at Strathclyde/experiment at INLN/detMOT5/'

# Define the matrix from intensity 2D profile for FFT. Image normally 1600*1200
matrix_size = 512 #1024
row_start = (600 - matrix_size)/2 #(1100 - matrix_size)/2
row_end = row_start + matrix_size
col_start = (800 - matrix_size)/2#(1645 - matrix_size)/2 #1650 for pump, 1645 for probe
...
#    signal = np.array(plt.imread(dname + 's'+str(i)+'.bmp'), dtype='float64')
    signal = np.array(plt.imread(dname + 'signal'+str(i)+'.bmp'), dtype='float64')
#    pump = np.array(plt.imread(dname + 'p'+str(i)+'.bmp'), dtype='float64')
    pump = np.array(plt.imread(dname + 'probe'+str(i)+'.bmp'), dtype='float64')
...
#    pump = np.sqrt(pump)
    normal = signal[img_ini:img_end,img_ini:img_end]
