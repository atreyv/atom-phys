...
def tof2(time,Data,Data_ind,atomic_mass,cycles,pixel_size,dname):  
 ...
 #binning and plotting
    ...
    plt.errorbar(times,tof_xs,yerr=error_x,xerr=None,ls='none',label='x',marker='o', color='blue')
    plt.errorbar(times,tof_ys,yerr=error_y,xerr=None,ls='none',label='z',marker='x', color='green')
    t = np.linspace(times[0],times[len(times)-1],len(times)*10)
    plt.plot(t,kinetic_expansion(Temp_x,sigma0x)(t),'b')
    plt.plot(t,kinetic_expansion(Temp_y,sigma0y)(t),'g')
    plt.xlabel('t (ms)')
    ...
    plt.text(times[1],tof_xs[len(tof_xs)-2],'$\mathrm{T_x = %d ~\mu K}$'%Temp_x)    
    plt.text(times[1],tof_xs[len(tof_xs)-3],'$\mathrm{T_z = %d ~\mu K}$'%Temp_y)
    plt.legend(loc=0)
    print ("$T_x = %d \mu K$ "%Temp_x)
    print ("$T_z = %d \mu K$ "%Temp_y)
